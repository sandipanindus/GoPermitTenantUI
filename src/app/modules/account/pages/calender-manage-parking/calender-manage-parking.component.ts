...
export class CalenderManageParkingComponent implements OnInit {
  ...
  // onbaynoset(event,bool){
  //   debugger
  //   if(event==null||event==undefined||event.length==0){

  //   }
  //   else{
  //     this.route.navigate(['/account/AssignBays', this.tenantid, this.bayno,this.getDateItem(event)]);
  //   }
  // }
  ...
  getvehicledetails(dateFormat: any) {
   ...
   // document.getElementById("parkingsession").style.display = 'none';
    ...
    this.service.getvehiclestimedetails(this.tenantid.toString(), this.bayno.toString()).subscribe((data: any) => {
      debugger;
      if (data.status == "200") {
        this.isshowblock = false;
  ...
  backToVisitor(){
    debugger
    this.route.navigate(['/account/AddVehicleRegistration']);
  }
